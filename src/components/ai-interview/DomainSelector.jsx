...
const DomainSelector = ({ onDomainSelect, loading }) => {
        ...
        const getDomains = async () => {
            try {
                const response = await axios.get(
                    `${config.apiBaseUrl}/api/interviewee/getAllDomains`
                );

                setAllDomains(await response.data);
            } catch (error) {
                setIsError(true);
            }
    ...
    if (isLoading)
        return (
            <p className="w-full text-semibold text-gray-400 text-center my-5">
                Loading...
            </p>
        );
    ...
    if (isError)
        return (
            <p className="w-full text-semibold text-red-400 text-center my-5">
                An error occured trying to load your domains. Go back to the last page.
            </p>
        );
